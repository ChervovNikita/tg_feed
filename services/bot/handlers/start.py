...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "2️⃣ Нажми /next чтобы получить статью\n"
...
async def cmd_help(message: Message):
    ...
    await message.answer(
        ...
        "<b>/next</b>\n"
        "Получить следующую статью. После реакции автоматически покажется следующая.\n\n"
        "<b>/tags</b>\n"
        ...
        "👍 - нравится (такие статьи будут чаще)\n"
        "👎 - не нравится (такие статьи будут реже)\n"
        "⏭️ - пропустить (без влияния на рекомендации)\n\n"
        "Чем больше реакций, тем лучше рекомендации!",
        parse_mode="HTML"
    )
